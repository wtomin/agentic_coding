...
- Inputs collected
  - input_folder: `mindone/transformers/models/{model-name}`

- Immediate checks
  - Verify `mindone/transformers/models/{model-name}` exists (warn user if it is empty)
  - Verify `tests/transformers_tests/models/{model-name}/` exists (create it if it is empty)

- Scope
  - Read all Python files under `mindone/transformers/models/{model-name}`
  - Read all arguments of `mindone/transformers/models/{model-name}/configuration_{model-name}.py`
...
```
  - Reduce the argument values (e.g., num_hidden_layers, vocab_size, batch_size, seq_len) in `configuration_{model-name}.py` to initiate a reduced-size model for testing
  - Write the test script to `tests/transformers_tests/models/{model-name}/test_modeling_{model-name}.py`
